...
<?php foreach ( $quizzes as $k => $v ) : ?>
	...
	// Additional Code for Adding Speaking Quiz in List 
	if(isset($v['ielts_quiz_id']) && intVal($v['ielts_quiz_id'])){
		$ielts_quiz_id = $v['ielts_quiz_id'];
		$try_number =  $v['ielts_quiz_try'];
		$quiz_url = learndash_get_step_permalink( $quiz->ID, $quiz_course_id );
		$result_url = ResultsManager::get_quiz_result_page_url($ielts_quiz_id,$try_number,$user_id);
		echo "<strong><a href='$quiz_url' target='_blank'>$quiz_title</a><strong> - Speaking Quiz <a href='$result_url' target='_blank'>Click Here to See Result</a>";
		continue;
	}
