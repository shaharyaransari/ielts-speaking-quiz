...
class CPTBase
	{
        // Utitliy Class for Generating Custom Post Types 
		...
		/**
     * Returns the Array of Post Objects on Success otherwise false.
     * Search Term Parameter can accept ID of post.
     */
    public static function search($post_type, $search_term ){
        $search_term = trim($search_term);
        $is_id_searched = false;
        $post_ids = array();
        $args = array(
            'post_type' => $post_type,
            'posts_per_page' => -1,
        ...
        // check if search term is Integer
        if(is_int($search_term)){
            $post_id = $search_term; // means id is searched
            $args['p'] = $post_id;
        }else{
            $args['s'] = $search_term;
        }
